...
function validateStableElementId(element, component, slot, errors) {
  ...
  if (!elementId || !/^[A-Za-z][A-Za-z0-9]*(?:[._-][A-Za-z0-9]+)+$/.test(elementId)) {
    errors.push(error("component_unstable_element_id", "Element-ID muss statisch und technisch deklariert sein.", context));
  }
  if (element?.stableIdSource !== "declaration") {
    errors.push(error("component_unstable_element_id", "Element-ID muss aus der Quelldeklaration stammen.", context));
  }
}
...
function validateRequirements(element, component, slot, errors) {
  ...
  const context = { componentId: component.componentId, slotId: slot.slotId, elementId: text(element.id) };
  if (requirements.textResize === true && !operations.includes("textResize")) {
    errors.push(error("component_required_text_resize_missing", "Textslot verlangt textResize, die Capability fehlt.", context));
  }
  if (requirements.move === true && !operations.includes("move")) {
    errors.push(error("component_required_move_missing", "Verschiebbarer Slot verlangt move, die Capability fehlt.", context));
  }
}
...
function validateUiComponentContracts({ components, registryElements = null, supportedOperations = null } = {}) {
  ...
  for (const component of aggregate.components) {
    ...
    if (componentIds.has(component.componentId)) {
      errors.push(error("component_id_duplicate", "componentId ist doppelt deklariert.", { componentId: component.componentId }));
    }
    ...
    for (const slot of component.slots) {
      ...
      if (!UI_COMPONENT_REFERENCE_KINDS.includes(slot.referenceKind)) {
        errors.push(error("component_reference_kind_invalid", "Slot braucht single- oder multi-Ref-Semantik.", { componentId: component.componentId, slotId: slot.slotId }));
      }
      if (!UI_COMPONENT_PRESENCE_MODES.includes(slot.presence)) {
        errors.push(error("component_presence_invalid", "Slot-Praesenz ist ungueltig.", { componentId: component.componentId, slotId: slot.slotId }));
      }
      ...
      if (elementIds.has(elementId)) {
        errors.push(error("component_element_id_duplicate", "Element-ID ist komponentenuebergreifend doppelt.", { componentId: component.componentId, slotId: slot.slotId, elementId }));
      }
      elementIds.add(elementId);
      if (!text(slot.element.refKey)) {
        errors.push(error("component_slot_reference_missing", "Registryziel besitzt keinen expliziten refKey.", { componentId: component.componentId, slotId: slot.slotId, elementId }));
      }
      if (!isObject(slot.element.baseline)) {
        errors.push(error("component_baseline_missing", "Registryziel besitzt keine Baseline.", { componentId: component.componentId, slotId: slot.slotId, elementId }));
      }
      ...
      if (Array.isArray(supportedOperations)) {
        for (const operation of allowedOps) {
          if (!supportedOperations.includes(operation)) {
            errors.push(error("component_capability_unsupported", "Capability besitzt keinen Hostweg.", { componentId: component.componentId, slotId: slot.slotId, elementId, operation }));
          }
        }
      }
    ...
    for (const slotId of component.requiredSlots) {
      ...
      if (!slot || slot.required === false) {
        errors.push(error("component_required_slot_missing", "Verpflichtender Komponentenslot fehlt.", { componentId: component.componentId, slotId }));
      }
    }
  }
  ...
  for (const element of aggregate.elements) {
    if (element.parentId != null && !byElementId.has(element.parentId)) {
      errors.push(error("component_parent_missing", "Parent des Komponenten-Slots fehlt.", { componentId: element.componentId, slotId: element.slotId, elementId: element.id, parentId: element.parentId }));
    }
  }
  ...
  if (Array.isArray(registryElements)) {
    ...
    for (const element of aggregate.elements) {
      if (!registryIds.has(element.id)) errors.push(error("component_registry_target_missing", "Komponentenziel fehlt in der aggregierten Registry.", { componentId: element.componentId, slotId: element.slotId, elementId: element.id }));
    }
    for (const registryId of registryIds) {
      if (!byElementId.has(registryId)) errors.push(error("component_registry_target_outside_contract", "Registryziel gehoert zu keinem Komponentenvertrag.", { elementId: registryId }));
    }
  }
...
function validateUiComponentReferenceBindings({ components, bindings, componentIds } = {}) {
  ...
  for (const component of contract.components) {
    ...
    for (const slot of component.slots) {
      ...
      if (slot.referenceKind === "single" && targetCount !== 1) {
        errors.push(error(targetCount > 1 ? "component_single_ref_duplicate" : "component_ref_unresolved", targetCount > 1 ? "Einzel-Ref loest mehrere Ziele auf." : "Einzel-Ref findet kein vorhandenes Ziel.", { ...context, targetCount }));
      }
      if (slot.referenceKind === "multi" && mountedInstanceCount > 0 && targetCount < 1) {
        errors.push(error("component_multi_ref_missing", "Multi-Ref findet trotz sichtbarer Instanzen kein Ziel.", { ...context, mountedInstanceCount, targetCount }));
      }
      if (binding.referenceResolved !== true && !(slot.referenceKind === "multi" && mountedInstanceCount === 0)) {
        errors.push(error("component_ref_unresolved", "Expliziter Ref-Resolver konnte das Ziel nicht aufloesen.", context));
      }
      if (slot.requirements?.directSelection === true && targetCount > 0) {
        ...
        if (childIndex < 0 || (parentIndex >= 0 && childIndex > parentIndex)) {
          errors.push(error("component_child_selection_swallowed", "Sichtbares Kindziel wird bei direkter Auswahl vom Parent verschluckt.", { ...context, parentId: slot.element.parentId || null }));
        }
      }
    }
  }
