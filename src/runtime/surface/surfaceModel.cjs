...
const SUPPORTED_SURFACE_TYPES = Object.freeze([
  "ui-screen",
  "panel",
  "pdf-page",
  "canvas",
...
function isSupportedSurfaceType(surfaceType) {
...
function normalizeSurfaceElement(input) {
...
function normalizeSurfaceModel(input) {
  const source = isObject(input) ? input : {};
...
function validateSurfaceElement(input) {
  const errors = [];
...
function validateSurfaceModel(input) {
  const errors = [];
