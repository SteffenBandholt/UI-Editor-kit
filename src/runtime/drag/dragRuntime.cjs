...
const SUPPORTED_DRAG_COORDINATE_SYSTEMS = Object.freeze([
  "css-pixels",
  "pdf-points",
  "canvas-pixels",
]);
...
function isSupportedDragCoordinateSystem(coordinateSystem) {
  return SUPPORTED_DRAG_COORDINATE_SYSTEMS.includes(coordinateSystem);
}

function normalizeDragBounds(input) {
...
function normalizeDragDelta(input) {
  const source = isObject(input) ? input : {};
  return {
    ...source,
    x: normalizeFiniteNumber(source.x),
...
function validateDragBounds(input) {
  const errors = [];
...
function validateDragDelta(input) {
  const errors = [];
...
function applyDragDelta(bounds, delta) {
...
function clampBoundsToConstraints(bounds, constraints = null) {
  const normalizedBounds = normalizeDragBounds(bounds);
...
function buildDragResult(input) {
  const source = isObject(input) ? input : {};
