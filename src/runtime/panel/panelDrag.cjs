...
const {
  ...
  validateDragBounds,
  validateDragDelta,
} = require("../drag/dragRuntime.cjs");

const PANEL_DRAG_COORDINATE_SYSTEM = "css-pixels";
...
function normalizePanelDragInput(input = {}) {
...
function buildPanelDragResult(input = {}) {
  const source = isObject(input) ? input : {};
...
function calculatePanelDragPosition(input = {}) {
