...
function createSelectionController(options = {}) {
  ...
  function refreshHover() {
    ...
    try {
      ...
      hoverOverlay.show({ ref: match.ref, target: match.target, document: options.document });
      emitState();
  ...
  function syncWithSelection() {
    try {
      ...
      if (selectedElementId && isElementLike(ref)) selectedOverlay.show({ ref, target, document: options.document }); else selectedOverlay.clear();
  ...
  function onViewportChange() { try { hoverOverlay.update(); selectedOverlay.update(); } catch (cause) { report(options, host, makeError(SelectionRuntimeErrorCodes.OVERLAY_FAILED, "Overlay-Aktualisierung ist fehlgeschlagen.", cause)); } }
  ...
  function start() {
    ...
    active = true; add(root, "pointermove", onPointerMove); add(root, "pointerleave", onPointerLeave); add(root, "click", onClick, true); add(root, "keydown", onKeyDown); add(root, "scroll", onViewportChange); const win = options.window || (options.document && options.document.defaultView); if (win && win.addEventListener) add(win, "resize", onViewportChange);
