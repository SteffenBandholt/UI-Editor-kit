...
function resizeTableColumnBoundary(table, intent = {}) {
  ...
  if (nextLeft < left.minimumWidth - 0.000001) return { ok: false, errors: [error("table_boundary_left_minimum", "delta", `${left.displayName} kann nicht schmaler als ${left.minimumWidth} werden.`)] };
  if (nextLeft > left.maximumWidth + 0.000001) return { ok: false, errors: [error("table_boundary_left_maximum", "delta", `${left.displayName} kann nicht breiter als ${left.maximumWidth} werden.`)] };
  if (nextRight < right.minimumWidth - 0.000001) return { ok: false, errors: [error("table_boundary_right_minimum", "delta", `${right.displayName} kann nicht schmaler als ${right.minimumWidth} werden.`)] };
  if (nextRight > right.maximumWidth + 0.000001) return { ok: false, errors: [error("table_boundary_right_maximum", "delta", `${right.displayName} kann nicht breiter als ${right.maximumWidth} werden.`)] };
  ...
  if (Math.abs(beforeTotal - afterTotal) > 0.000001) return { ok: false, errors: [error("table_boundary_total_changed", "delta", "Die feste Tabellenbreite würde sich durch die Grenzverschiebung ändern.")] };
  const next = normalizeTableLayout({ ...model, columns });
  return Object.freeze({ ok: true, changed: true, model: next, metrics: measureTableLayout(next),
