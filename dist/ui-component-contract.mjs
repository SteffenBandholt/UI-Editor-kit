...
function validateBounds(element, component, slot, errors) {
  ...
  function validateOptionalRange(minKey, maxKey, dimension) {
    ...
    if (minimumDeclared && !finite(baseline[minKey])) errors.push(error("component_geometry_bound_invalid", `${minKey} muss eine endliche Zahl oder unbegrenzt sein.`, { ...context, dimension, bound: minKey }));
    if (maximumDeclared && !finite(baseline[maxKey])) errors.push(error("component_geometry_bound_invalid", `${maxKey} muss eine endliche Zahl oder unbegrenzt sein.`, { ...context, dimension, bound: maxKey }));
