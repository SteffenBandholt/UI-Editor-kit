...
const SPACING_OPERATIONS = Object.freeze(["spacingIncrease", "spacingDecrease", "spacingSet", "spacingReset"]);
...
function validateSpacingIntent(operation, payload, supportedTargets = SPACING_TARGETS) {
  ...
  if (operation !== "spacingReset" && (!Number.isFinite(Number(payload.spacing.value)) || Number(payload.spacing.value) < 0)) errors.push({ code: "spacing_value_invalid", field: "payload.spacing.value" });
  return { ok: errors.length === 0, errors };
}

export { SPACING_TARGETS, SPACING_OPERATIONS, WIDTH_FLOW_ACTIONS, normalizeSpacingValues, validateSpacingIntent };
export default Object.freeze({ SPACING_TARGETS, SPACING_OPERATIONS, WIDTH_FLOW_ACTIONS, normalizeSpacingValues, validateSpacingIntent });
