...
## Bestands-App-Registrierung und Registry-Refresh ab M80.1

Die Ziel-App ist Eigentümerin und einzige Wahrheitsquelle ihrer Registry. Der Editor erkennt keine UI selbst. Beim Bestands-App-Anschluss werden Vertrag, Adapter, Registry, erwartetes Inventar, Ref-Auflösung, Baseline, Fachaktionsschutz und Vollständigkeitsstatus explizit geprüft. Zulässige Registrierungsstatus sind `notInstalled`, `registrationRequired`, `registrationInProgress`, `incomplete`, `complete`, `changed`, `incompatible` und `blocked`.
