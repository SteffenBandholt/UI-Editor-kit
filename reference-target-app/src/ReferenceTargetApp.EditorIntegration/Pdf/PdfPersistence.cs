...
public static class PdfLayoutProfileDocumentValidator
{
    ...
    public static (bool Success, string Code, string Message) Validate(PdfLayoutProfileDocument? document, PdfElementRegistry registry)
    {
        ...
        if (document.ApplicationId != PdfOrderDocumentRegistryFactory.ApplicationId) return Fail("applicationId passt nicht zur Ziel-App.");
        if (document.DocumentType != PdfOrderDocumentRegistryFactory.DocumentType) return Fail("PDF-Dokumenttyp ist falsch.");
        if (document.ProfileId != ProfileId) return Fail("PDF-profileId ist falsch.");
        if (document.ScopeId != PdfRegistryIds.Scope || document.LayoutState?.ScopeId != PdfRegistryIds.Scope) return Fail("PDF-Scope ist falsch.");
        ...
        foreach (var definition in expected)
        {
            ...
            if (state.ScopeId != PdfRegistryIds.Scope || state.ElementId.StartsWith("ui.", StringComparison.Ordinal)) return Fail("UI-ID oder falscher Scope im PDF-Profil.");
    ...
    private static bool AllowedFieldsMatch(PdfElementDefinition definition, PdfElementLayoutState state) =>
    ...
    private static bool Finite(PdfElementLayoutState state) =>
        new[] { state.X, state.Y, state.Width, state.Height, state.TextOffsetX, state.TextOffsetY, state.FontSize }
...
public sealed class AtomicJsonPdfLayoutProfileStore
{
    ...
    public AtomicJsonPdfLayoutProfileStore(string applicationLayoutRoot)
    {
        ...
        FilePath = Path.Combine(RootDirectory, "pdf-standard.pdf-layout.json");
    }
    ...
    public async Task<PdfProfileSaveResult> SaveAsync(PdfElementRegistry registry, PdfLayoutState state, CancellationToken cancellationToken = default)
    {
        var document = new PdfLayoutProfileDocument(PdfLayoutProfileDocumentValidator.SchemaVersion,
            ...
            PdfRegistryIds.Scope, DateTimeOffset.UtcNow, PdfRegistryFingerprint.Create(registry), Clone(state));
